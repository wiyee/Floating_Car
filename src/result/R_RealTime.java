...
public class R_RealTime {
	private static String startTime = "2015-02-02 07:00:00";
		...
		for (int i = 15; i <= 15; i += 5) {
			...
			for (int roadId : roadIdSet) {
				...
				for (long fromMs : fromMsList) {
					...
					// System.out.println(roadId + " " + speed1 + " " + speed +
					// " " + foreSpeed);
					...
					if (!roadList.contains(roadId)) {
						...
						if (isRightLocation(Double.parseDouble(lonlat[2]), Double.parseDouble(lonlat[3]))) {
							write2DB(roadId, lonlat, speed, foreSpeed, i);
						}
						
					}
					// System.out.println(roadId + " name:" + lonlat[3] + " " +
					// speed + " " + foreSpeed + " first:" + lonlat[0] + "
					// center:" + lonlat[1] + " end:" + lonlat[2]);
				}
			}
	...
	private static void write2DB(int roadId, String[] lonlat, double realSpeed, double preSpeed, int i) {
	...
	public static String[] getRoadName(int id) {
		...
		try {
			...
			String sql = "select NAME,FIRST_JD,FIRST_WD,CENTER_JD,CENTER_WD,END_JD,END_WD from MAP_BAIDU where ID = "
					+ id;
	...
	public static void main(String[] args) {
		...
		System.out.println("time:" + (end_time - start_time) / 1000f + "s");
	}
